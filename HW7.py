...
# Your name: Claire Callen
# Your student id: 4291 6162
# Your email: claireca@umich.edu
# List who you have worked with on this project: Julianna Ross
...
def make_players_table(data, cur, conn):
    ...
    for player in data['squad']:
        ...
        birth = player['dateOfBirth'].split("-")[0]
        year = int(birth)
        cur.execute("INSERT OR IGNORE INTO Players (id, name, position_id, birthyear, nationality) VALUES (?, ?, ?, ?, ?)",(player['id'], player['name'], positionid, year, player['nationality']))
    conn.commit()
...
def birthyear_nationality_search(age, country, cur, conn):
    final_lst = []
    cur.execute('SELECT name, nationality, birthyear FROM Players WHERE Players.nationality = ?', [country])
    for row in cur:
        if 2023 - age > row[-1]:
            final_lst.append(row)
    return final_lst
...
def position_birth_search(position, age, cur, conn):
    final_lst = []
    cur.execute('SELECT Players.name, Positions.position, Players.birthyear FROM Players JOIN POSITIONS ON Players.position_id = Positions.id WHERE Positions.position = ?', [position])
    for row in cur:
        if 2023 - age < row[-1]:
            final_lst.append(row)
    return final_lst
